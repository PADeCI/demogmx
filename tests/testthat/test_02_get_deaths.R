...
test_that("correct inputs are valid", {
  # Valid parameter set
  expect_silent(get_deaths(v_state = c("National", "Mexico City"),
                           v_year = 1976,
                           v_sex = "Total",
                           v_age = c(10, 25, 55, 75),
                           age_groups = TRUE))
...
test_that("state inputs are correct", {
  # Error in v_state
  expect_error(get_deaths(v_state = "national", #National <- correct
                          v_year = 1976,
                          v_sex = "Total",
                          v_age = c(10, 25, 55, 75),
                          age_groups = TRUE),
  ...
  # Not an exact match
  expect_error(get_deaths(v_state = c("National", "Ags"), # Aguascalientes
                          v_year = 1976,
                          v_sex = "Total",
                          v_age = c(10, 25, 55, 75),
                          age_groups = TRUE),
  ...
  # Not a character
  expect_error(get_deaths(v_state = 65,
                          v_year = 1976,
                          v_sex = "Total",
                          v_age = c(10, 25, 55, 75),
                          age_groups = TRUE),
...
test_that("year inputs are correct", {
  ...
  expect_error(get_deaths(v_state = c("National", "Aguascalientes"),
                          v_year = 1905,
                          v_sex = "Total",
                          v_age = c(10, 25, 55, 75),
                          age_groups = TRUE),
...
test_that("age groups are correct", {
  ...
  expect_warning(get_deaths(v_state = c("National", "Aguascalientes"),
                            v_year = c(2000, 2010, 2025),
                            v_sex = "Total",
                            v_age = seq(0, 15),
...
test_that("the output is correct when all the available vars are selected", {
  ...
  df_gd <- get_deaths(v_state = unique(df_mortrate_state_age_sex$state),
  ...
  # It should have the same number of rows of the original deaths dataset
  expect_equal(dim(df_gd)[1], dim(df_mortrate_state_age_sex)[1])
})
...
test_that("the ungrouped output has the correct dimensions", {
  # Ungrouped
  df_gd <- get_deaths(v_state = c("Guerrero", "Mexico City"),
                       v_year = seq(from = 1970, to = 2050, by = 1),
  ...
  expect_equal(dim(df_gd)[1], size_grp)
